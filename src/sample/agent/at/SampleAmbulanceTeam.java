...
import sample.agent.AbstractSampleAgent;
import sample.util.DistanceSorter;

/**
 * A sample ambulance team agent.
 * 市民の救助を行います．
 * move, load, unload, rescueコマンドを使用できます．
 */
public class SampleAmbulanceTeam extends AbstractSampleAgent<AmbulanceTeam> {
	...
	protected void postConnect() {
		...
		Logger.info(toString() + " was connected. [id=" + getID() + "]");
	}
	...
	private List<Human> getTargets() {
		...
		for (StandardEntity next : model.getEntitiesOfType(StandardEntityURN.CIVILIAN, StandardEntityURN.FIRE_BRIGADE, StandardEntityURN.POLICE_FORCE, StandardEntityURN.AMBULANCE_TEAM)) {
			...
			if (h.isHPDefined() && h.isBuriednessDefined() && h.isDamageDefined() && h.isPositionDefined() && h.getHP() > 0 && (h.getBuriedness() > 0 || h.getDamage() > 0)) {
				targets.add(h);
			}
		}
