...
public final class LaunchAgents {
	 ...
	 * 
	 * @param args
	 * プログラム引数は以下のように記述してください:
	 * <code>[-p <port>]? [-h <hostname>]? [-c <config file>]* (fully.qualified.classname[*multiplier])+</code>
	 * <ul>
	 * 	<li>"?"は必須ではないことを示します．</li>
	 * 	<li>"*"は0個以上を示します．</li>
	 * 	<li>"+"は一つ以上を示します．</li>
	 *	</ul>
	 *	multiplierにはエージェント数を入力してください．
	 *	nを指定すると最大数まで起動します．
	 */
	...
	private static void connect(ComponentLauncher launcher, String argLine,
			boolean gui) throws InterruptedException, ConnectionException {
